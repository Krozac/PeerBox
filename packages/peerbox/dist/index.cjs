...
class World {
  ...
  getEntitiesWithComponent(name) {
    return this.getEntities().filter((id) => {
      var _a;
      return (_a = this.components.get(id)) == null ? void 0 : _a.has(name);
    });
  }
  ...
  removeComponent(entityId, name) {
    var _a;
    (_a = this.components.get(entityId)) == null ? void 0 : _a.delete(name);
    this.emit("componentRemoved", { entityId, name });
  }
...
function userComponent({ id, name = "Default User", connected = true }) {
  return {
    id,
    name,
    role: "client",
...
async function tokenize(secret, params) {
...
function createTextCleaner(words) {
...
function normalize(str) {
  return str.normalize("NFD").replace(zeroWidthRegex, "").replace(/(.)\1{2,}/g, "$1$1");
}
...
const LoadingOverlay = /* @__PURE__ */ (function createLoadingOverlayModule() {
  ...
  let shown = false;
  let root = null;
  function ensure(customRoot) {
    root = customRoot || document.body;
  ...
  function show(text = "Loading…", customRoot) {
    ensure(customRoot);
...
class SceneManager {
  ...
  async goBack(transition = null) {
    if (this.previousScene) {
      ...
      this.previousScene = null;
      return this.load(scene, context, transition);
    }
  }
}
class TransitionManager {
  ...
  async out(name, root) {
    ...
    console.log(t);
    if (t == null ? void 0 : t.out) await t.out(root);
  }
  async in(name, root) {
    const t = this.transitions.get(name);
    if (t == null ? void 0 : t.in) await t.in(root);
  }
}
function SyncSystem({ world, network, isHost = false }) {
  ...
  function onAction(type, handler, { key } = {}) {
    if (!actionHandlers.has(type)) {
      actionHandlers.set(type, /* @__PURE__ */ new Map());
    }
  ...
  function onError(type, handler, { key } = {}) {
    if (!errorHandlers.has(type)) {
      errorHandlers.set(type, /* @__PURE__ */ new Map());
    }
  ...
  function broadcast(actionType, payload, opts = {}, seq2, clientEntityId, clientId) {
    if (!isHost) {
  ...
  network.on("sync", ({ clientId, clientEntityId, action, payload, seq: seq2 }) => {
    ...
    if (isHost) {
      let result;
      for (const handler of handlers.values()) {
        const r = handler({ world, payload, clientId, seq: seq2 });
        if (r !== void 0 && result === void 0) {
          result = r;
        }
      }
      ...
      if (result && result.broadcast !== false) {
        broadcast(result.action, result.payload ?? payload, result.opts, seq2, clientEntityId, clientId);
      }
    } else {
      for (const handler of handlers.values()) {
        handler({
      ...
      if (pendingPredictions.has(key)) {
        ...
        for (const [k, p] of pendingPredictions.entries()) {
          ...
          if (cid === clientEntityId && Number(s) > Number(seq2)) {
            const handlers2 = actionHandlers.get(p.action);
            if (!handlers2 || handlers2.size === 0) continue;
            for (const handler of handlers2.values()) {
              handler({
                world,
                payload: p.payload,
                clientId,
